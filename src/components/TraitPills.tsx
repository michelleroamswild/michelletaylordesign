...
export default function TraitPills({ traits }: { traits: string[] }) {
  return (
    <div className="flex justify-center mt-20">
      <div className="flex items-stretch w-full bg-white/80 backdrop-blur-sm border rounded-full overflow-hidden" style={{ borderColor: "hsl(249, 60%, 80%)" }}>
        {traits.map((trait, i) => (
          <div
            key={trait}
            className="flex flex-1 items-center justify-center gap-2.5 text-base text-muted px-6 py-3.5"
            style={{
              borderLeft: i > 0 ? "1px solid hsl(249, 60%, 80%)" : undefined,
            }}
          >
